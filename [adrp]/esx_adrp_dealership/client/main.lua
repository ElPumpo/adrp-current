...
RegisterNetEvent('esx_adrp_dealership:setVehiclesForSale')
AddEventHandler('esx_adrp_dealership:setVehiclesForSale', function(_availableVehiclesForSale)
	availableVehiclesForSale = _availableVehiclesForSale
end)

RegisterNetEvent('esx_adrp_dealership:relayMyNetworkID')
AddEventHandler('esx_adrp_dealership:relayMyNetworkID', function(_myNetworkID)
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'sell_vehicle', {
					...
					elseif data.current.action == 'sell' then
						...
						if price > 0 then
							...
							ESX.TriggerServerCallback('esx_adrp_dealership:addVehicle', function(success)
								if success then
...
function openDealershipMenu()
	...
	for k,v in pairs(availableVehiclesForSale) do
		...
		if IsModelInCdimage(vehicleProps.model) then
			...
			print(('esx_adrp_dealership: ignoring invalid vehicle model "%s" from networkID "%s"'):format(vehicleProps.model, v.networkID))
		end
	end
...
function openOwnerMenu(parentMenu, currentData)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'owner_menu', {
		...
		if data.current.action == 'change' then
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'owner_menu_price', {
				...
				if vehiclePrice then
					ESX.TriggerServerCallback('esx_adrp_dealership:changePrice', function(success)
						if success then
		...
		elseif data.current.action == 'unlist' then
			ESX.TriggerServerCallback('esx_adrp_dealership:unlistVehicle', function(success)
				if success then
...
function openBuyMenu(parentMenu, currentData)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'owner_menu', {
		...
		if data.current.confirm then
			ESX.TriggerServerCallback('esx_adrp_dealership:buyVehicle', function(success)
				if success then
					parentMenu.removeElement({plate = currentData.plate})
					parentMenu.refresh()
					menu.close()
