	...
	while true do
		...
		if IsControlJustReleased(0, 57) then
			if IsInputDisabled(0) and CheckPolice() then
				...
				TriggerServerEvent('adrp_policemdt:sendGlobalPoliceMessage', message)
				TriggerServerEvent('adrp_policemdt:sendGlobalPoliceWaypoint', coords.x, coords.y)
			end
		end
	end
end)

RegisterNetEvent('adrp_policemdt:sendGlobalPoliceWaypoint')
