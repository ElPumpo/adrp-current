...
RegisterServerEvent('adrp_policemdt:sendGlobalPoliceMessage')
AddEventHandler('adrp_policemdt:sendGlobalPoliceMessage', function(message)
	for playerId,_ in pairs(availableUnits) do
		TriggerClientEvent('chat:addMessage', playerId, {args = {'', message}})
	end
end)

RegisterServerEvent('adrp_policemdt:sendGlobalPoliceWaypoint')
AddEventHandler('adrp_policemdt:sendGlobalPoliceWaypoint', function(x, y)
	for playerId,_ in pairs(availableUnits) do
		TriggerClientEvent('adrp_policemdt:sendGlobalPoliceWaypoint', playerId, x, y)
	end
end)
