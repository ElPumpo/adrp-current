...
RegisterNetEvent('adrp_reportsystem:report')
AddEventHandler('adrp_reportsystem:report', function(text)
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'report', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'report_confirm', {
			...
			if action == 1 then
				TriggerEvent('adrp_reportsystem:report', data.value)
...
RegisterNetEvent('adrp_reportsystem:openReports')
AddEventHandler('adrp_reportsystem:openReports', function()
	if not admin then
		openReports2()
	end
end)
...
RegisterNetEvent('adrp_reportsystem:notifyStaff')
AddEventHandler('adrp_reportsystem:notifyStaff', function(text)
