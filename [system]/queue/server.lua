...
Citizen.CreateThread(function()
	...
	while true do
		...
		if #waiting > 0 and #connecting + countOnlinePlayers() < maxServerSlots then
			ConnectFirst()
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		-- if the size is 0 then this native will break if you input a
		-- number. thats why we turn it into a string
		SetConvarServerInfo('Players in Queue', SizeOfTable(players) .. '')
	end
end)
...
function GetMessage(steamID)
	...
	if GetPoints(steamID) ~= nil then
		msg = Config.Position .. GetPlace(steamID) .. "/".. #waiting .. " " .. "\n"
