...
AddEventHandler('rlUpdateNamesResult', function(res)
	if source ~= trustedPlayer then
		DropPlayer(source, 'rconlog: you have been kicked because your host id cannot be found. This is also known as a ghost server, retry joining again.')
		return
	end
...
AddEventHandler('rconCommand', function(commandName, args)
	if commandName:lower() == 'status' then
	...
	elseif commandName:lower() == 'clientkick' then
		...
		if playerId then
			...
			print('rconlog: invalid usage. Correct usage: clientkick [id] [reason]')
		end
