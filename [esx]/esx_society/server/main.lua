...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, type)
	...
	local _identifier = GetPlayerIdentifiers(source)[1]

	local message = (('**Source:** %s | %s\n**Action:** %s'):format(source, _identifier, type))
	TriggerEvent('adrp_anticheat:sendLog', message, 'Society Set Job Event Used')
	...
	if isBoss then
		...
		print(('esx_society: %s attempted to setJob'):format(_identifier))

		local message = (('esx_society setJob cheater detected!\n\nSource: %s | %s'):format(source, _identifier))
...
function isPlayerBoss(playerId, job)
	...
	if xPlayer.job.name == job and xPlayer.job.grade_name == 'boss' then
		...
		local message = (('esx_society isBoss cheater detected!\n\nSource: %s | %s'):format(playerId, xPlayer.identifier))
		TriggerEvent('adrp_anticheat:cheaterDetected', message)
