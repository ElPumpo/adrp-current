...
RegisterServerEvent('esx_billing:sendPlayerBill')
AddEventHandler('esx_billing:sendPlayerBill', function(playerId, sharedAccountName, label, amount)
		...
		elseif not account then
			...
			if xTarget then
				MySQL.Async.execute('INSERT INTO billing (identifier, sender, target_type, target, label, amount) VALUES (@identifier, @sender, @target_type, @target, @label, @amount)', {
					...
					TriggerClientEvent('esx:showNotification', xTarget.source, _U('received_invoice'))
					TriggerClientEvent('customNotification', _source, "You have sent an invoice for "..label)
				end)
			end
		end
	end)
