...
RegisterServerEvent('esx_jobs:startWorking')
AddEventHandler('esx_jobs:startWorking', function(zoneIndex)
...
RegisterNetEvent('esx_jobs:processCaution')
AddEventHandler('esx_jobs:processCaution', function(cautionType, cautionAmount, spawnPoint, vehicle)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if cautionType == 'take' then
		if cautionAmount <= Config.MaxCaution and cautionAmount > 0 then
			...
			print(('esx_jobs: %s is using a lua injector!'):format(identifier))
		
			local message = ('**Source:** %s | %s\n**Caution Amount**: %s'):format(source, identifier, cautionAmount)
			TriggerEvent('adrp_anticheat:cheaterDetected', message, 'esx_jobs: cheater ran esx_jobs:processCaution with caution amount above 10k!')
	...
	elseif cautionType == 'give_back' then
		if cautionAmount <= 1 and cautionAmount > 0 then
			...
			local message = ('**Source:** %s | %s\n**Vehicle Damage:** %s'):format(source, identifier, cautionAmount)
			TriggerEvent('adrp_anticheat:cheaterDetected', message, 'esx_jobs: cheater ran esx_jobs:processCaution with invalid vehicle damage multiplier!')
			TriggerEvent('adrp_anticheat:banCheater', source)
		end
	end
end)
