...
AddEventHandler('esx_jobs:action', function(job, zone, zoneIndex)
	...
	elseif zone.Type == 'work' then
		...
		if IsPedOnFoot(playerPed) then
			TriggerServerEvent('esx_jobs:startWorking', zoneIndex)
		else
					...
					if IsPedInAnyVehicle(playerPed, false) then
						...
						if playerPed == driverPed then
							if myPlate[plate] then
								...
								TriggerServerEvent('esx_jobs:processCaution', 'give_back', giveBack, 0, 0)
...
function spawnVehicle(spawnPoint, vehicle, vehicleCaution)
	hintToDisplay = nil
	hintIsShowed = false
	TriggerServerEvent('esx_jobs:processCaution', 'take', vehicleCaution, spawnPoint, vehicle)
end
