...
RegisterNetEvent('esx_tattooshop:load')
AddEventHandler('esx_tattooshop:load', function(tattooList)
	...
	if tattooList then
		...
		for _,k in pairs(currentTattoos) do
			ApplyPedOverlay(playerPed, GetHashKey(k.collection), GetHashKey(Config.TattooList[k.collection][k.texture].nameHash))
		end
	end
end)
...
function drawTattoo(current, collection)
	SetEntityHeading(PlayerPedId(), 297.7)
