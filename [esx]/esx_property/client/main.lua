...
AddEventHandler('esx_property:sendProperties', function(properties)
	...
	ESX.TriggerServerCallback('esx_property:getOwnedProperties', function(ownedProperties)
		for i=1, #ownedProperties, 1 do
			SetPropertyOwned(ownedProperties[i].name, true, ownedProperties[i].rented)
		end
	end)
end)
...
function CreateBlips()
	for k,property in ipairs(Config.Properties) do
		if property.entering then
			if not Config.HiddenProperties[property.name] then
				...
				AddFancyBlipInfo(Blips[property.name], property)
			end
		end
	end
end

function AddFancyBlipInfo(blip, property)
	exports['fivem-blipapi']:ResetBlipInfo(blip)
	exports['fivem-blipapi']:SetBlipInfoTitle(blip, property.label, false)

	local price, buyText, rentText

	if property.isGateway then
		local childProperties, childPrices = GetGatewayProperties(property), {}

		for k,v in ipairs(childProperties) do
			table.insert(childPrices, v.price)
		end

		table.sort(childPrices) -- Sort list of prices, from lowest to highest number (chronological)
		price = childPrices[1] -- Select lowest value

		buyText = ('From ~g~$%s~s~'):format(ESX.Math.GroupDigits(price))
		rentText = ('From ~g~$%s~s~'):format(ESX.Math.GroupDigits(ESX.Math.Round(price / 200)))
	else
		price = property.price
		buyText = ('~g~$%s~s~'):format(ESX.Math.GroupDigits(price))
		rentText = ('~g~$%s~s~'):format(ESX.Math.GroupDigits(ESX.Math.Round(price / 200)))
	end

	exports['fivem-blipapi']:AddBlipInfoText(blip, 'Buy price', buyText)
	exports['fivem-blipapi']:AddBlipInfoText(blip, 'Rent price', rentText)
	--exports['fivem-blipapi']:AddBlipInfoText(blip, 'Type', 'Apartment / TODO')

	--exports['fivem-blipapi']:AddBlipInfoHeader(blip, '') -- Empty header adds the header line
	--exports['fivem-blipapi']:AddBlipInfoText(blip, 'TODO')

	ESX.Streaming.RequestStreamedTextureDict('blip_properties')
	exports['fivem-blipapi']:SetBlipInfoImage(blip, 'blip_properties', ('property_%s'):format(property.name))
end
...
function SetPropertyOwned(name, owned, rented)
	...
	if owned then
		...
		AddFancyBlipInfo(Blips[enteringName], property)
	else
...
function OpenPropertyMenu(property)
	...
	if PropertyIsOwned(property) then
		...
		if OwnedProperties[property.name] then -- Rented
			table.insert(elements, {label = _U('leave'), value = 'leave'})
		else
			table.insert(elements, {label = _U('sell_property', ESX.Math.GroupDigits(ESX.Math.Round(property.price / 2))), value = 'leave'})
		end
	else
		...
		if PlayerData.gang and PlayerData.gang.name == 'cartel' and property.name == 'Senorard' then
			table.insert(elements, {label = _U('visit'), value = 'visit_cartel'})
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'property', {
		...
		elseif data.current.value == 'leave_cartel' then
			SetEntityCoords(PlayerPedId(), 1392.959, 1141.3348, 113.443, 0, 0, 0, 0)
		...
		elseif data.current.value == 'visit' then
			TriggerEvent('instance:create', 'property', {property = property.name, owner = ESX.GetPlayerData().identifier})
		elseif data.current.value == 'visit_cartel' then
			TriggerEvent('instance:create', 'property', {property = property.name, owner = ESX.GetPlayerData().identifier})
			--SetEntityCoords(PlayerPedId(), 1399.1635, 1142.0968, 113.333, 0, 0, 0, 0)
			--isOwned = true
			--HasChest = true
		end
		...
		CurrentActionMsg  = _U('press_to_menu', property.label)
...
function OpenGatewayOwnedPropertiesMenu(property)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_owned_properties', {
		title    = property.label .. ' - ' .. _U('owned_properties'),
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
		menu.close()
		...
		if OwnedProperties[data.current.value] then -- Rented
			table.insert(elements, {label = _U('leave'), value = 'leave'})
		else
			table.insert(elements, {label = _U('sell_property', ESX.Math.GroupDigits(ESX.Math.Round(data.current.price / 2))), value = 'leave'})
		end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_owned_properties_actions', {
			...
			elseif data2.current.value == 'leave' then
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_owned_properties', {
					...
					align    = 'top-left',
					elements = {
						{label = 'No', confirm = false},
						{label = 'Yes', confirm = true}
				}}, function(data3, menu3)
					menu3.close()

					if data3.current.confirm then
						menu2.close()
...
function OpenGatewayAvailablePropertiesMenu(property)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_available_properties', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_available_properties_actions', {
			...
			align    = 'top-left',
			elements = {
				{label = _U('buy', ESX.Math.GroupDigits(data.current.price)), value = 'buy'},
				{label = _U('rent', ESX.Math.GroupDigits(ESX.Math.Round(data.current.price / 200))), value = 'rent'}
		}}, function(data2, menu2)
			menu2.close()
...
function OpenRoomInventoryMenu(property, owner)
	...
	ESX.TriggerServerCallback('esx_property:getPropertyInventory', function(inventory)
		...
		if inventory.blackMoney > 0 then
			table.insert(elements, {label = _U('dirty_money', ESX.Math.GroupDigits(inventory.blackMoney)), type = 'item_account', value = 'black_money'})
		end
		...
		for i=1, #inventory.items, 1 do
			...
			if item.count > 0 then
				table.insert(elements, {label = item.label .. ' x' .. ESX.Math.GroupDigits(item.count), type = 'item_standard', value = item.name})
			end
		end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room_inventory', {
			...
			if data.current.type == 'item_weapon' then
				...
				TriggerServerEvent('esx_property:getItem', owner, data.current.type, data.current.value, nil)
...
function OpenPlayerInventoryMenu(property, owner)
	ESX.TriggerServerCallback('esx_property:getPlayerInventory', function(inventory)
		...
		for k,v in ipairs(inventory.loadout) do
			table.insert(elements, {
				label = ('%s [%s ammo]'):format(v.label, v.ammo),
				value = v.name,
				type = 'item_weapon'
			})
		end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_inventory', {
			...
			if data.current.type == 'item_weapon' then
				...
				TriggerServerEvent('esx_property:putItem', owner, data.current.type, data.current.value, nil)
...
AddEventHandler('instance:onEnter', function(instance)
	if instance.type == 'property' then
		...
		if isOwned or not isHost or property.name == 'Senorard' then
			HasChest = true
		else
			HasChest = false
		end
