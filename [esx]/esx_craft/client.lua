...
local function craftItem(ingredients)
	...
	TriggerServerEvent('esx_craft:craftItem', ingredientsPrepped)
end

RegisterNetEvent('esx_craft:openMenu')
AddEventHandler('esx_craft:openMenu', function(playerInventory)
	...
	Citizen.CreateThread(function()
		while true do
			...
			if IsControlJustReleased(1, Config.Keyboard.keyCode) and GetLastInputMethod(2) then
				TriggerServerEvent('esx_craft:getPlayerInventory')
			end
		end
	end)
end
...
if Config.Shop.useShop then
	...
	Citizen.CreateThread(function()
		...
		AddTextComponentSubstringPlayerName(Config.Shop.shopName)
		...
		while true do
			...
			if inDrawingRange then
				...
				if not display and isPlayerInRange(GetEntityCoords(PlayerPedId()), Config.Shop.shopCoordinates, Config.Shop.zoneSize.x) then
					...
					AddTextComponentString('Press ~INPUT_CONTEXT~ to ~y~Craft an Item~s~.')
