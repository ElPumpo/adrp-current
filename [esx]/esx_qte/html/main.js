...
$(function () {
	...
	$(document).keydown(function (e) {
		if (active) {
			if (e.keyCode == 37 && button == "left" || e.keyCode == 38 && button == "up" || e.keyCode == 39 && button == "right" || e.keyCode == 40 && button == "down" || e.keyCode == 87 && button == "W" || e.keyCode == 65 && button == "A" || e.keyCode == 83 && button == "S" || e.keyCode == 68 && button == "D") {
				$.post('http://esx_qte/success');

			} else {
				$.post('http://esx_qte/fail');
			}
			active = false;
			$('.qte').fadeOut();
		}
