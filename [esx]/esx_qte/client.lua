...
AddEventHandler('qte:clientClicked', function() end)
AddEventHandler('qte:clientClickFailed', function() end)
...
AddEventHandler('qte:clientWait', function(arrow, time, succesCb, failCb)
	...
	while waiting and time2 < time/10 do
		time2 = time2 + 1
		Citizen.Wait(0)
	end
...
AddEventHandler('qte:start', function(buttons, gameDuration, qteDuration, animLib, animName, succesCb, failCb, finishCb)
	...
	TaskPlayAnim(playerPed, animLib, animName, 8.0, 1.0, -1, 1, 0.0, false, false, false)
...
RegisterNUICallback('success', function()
	TriggerEvent('qte:clientClicked')
	clicked = true
	waiting = false
...
RegisterNUICallback('fail', function()
	TriggerEvent('qte:clientClickFailed')
	clicked = false
	waiting = false
end)