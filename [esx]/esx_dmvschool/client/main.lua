...
function StopTheoryTest(success)
	...
	if success then
		TriggerServerEvent('esx_dmvschool:rewardLicense', 'dmv')
...
function StopDriveTest(success)
	if success then
		TriggerServerEvent('esx_dmvschool:rewardLicense', CurrentTestType)
		ESX.ShowNotification(_U('passed_test'))
	else
		ESX.ShowNotification(_U('failed_test'))
	end
...
function OpenDMVSchoolMenu()
	...
	if not ownedLicenses.dmv then
		table.insert(elements, {
			label = ('%s: <span style="color:#00ea1b;">$%s</span>'):format(_U('theory_test'), ESX.Math.GroupDigits(Config.Prices.dmv)),
			drive = 'theory_test'
		})
	end
	...
	if ownedLicenses.dmv then
		if not ownedLicenses.drive then
			table.insert(elements, {
				label = ('%s: <span style="color:#00ea1b;">$%s</span>'):format(_U('road_test_car'), ESX.Math.GroupDigits(Config.Prices.drive)),
				value = 'drive_test',
				drive = 'drive'
			})
		end
		...
		if not ownedLicenses.drive_bike then
			table.insert(elements, {
				label = ('%s: <span style="color:#00ea1b;">$%s</span>'):format(_U('road_test_bike'), ESX.Math.GroupDigits(Config.Prices.drive_bike)),
				value = 'drive_test',
				drive = 'drive_bike'
			})
		end
		...
		if not ownedLicenses.drive_truck then
			table.insert(elements, {
				label = ('%s: <span style="color:#00ea1b;">$%s</span>'):format(_U('road_test_truck'), ESX.Math.GroupDigits(Config.Prices.drive_truck)),
				value = 'drive_test',
				drive = 'drive_truck'
			})
		end

		if not ownedLicenses.taxi and ownedLicenses.drive then
			table.insert(elements, {
				label = ('Commercial Taxi License: <span style="color:#00ea1b;">$%s</span>'):format(ESX.Math.GroupDigits(Config.Prices.taxi_license)),
				value = 'taxi'
			})
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'dmvschool_actions', {
		...
		if data.current.drive == 'theory_test' then
			ESX.TriggerServerCallback('esx_dmvschool:payPrice', function(success)
				if success then
					menu.close()
					StartTheoryTest()
				end
		...
		elseif data.current.value == 'drive_test' then
			ESX.TriggerServerCallback('esx_dmvschool:payPrice', function(success)
				if success then
					StartDriveTest(data.current.drive)
				end
			end, data.current.drive)
		elseif data.current.value == 'taxi' then
			ESX.TriggerServerCallback('esx_dmvschool:buyTaxiLicense', function(bought)
				if bought then
					ESX.ShowNotification('You have bought the commercial taxi license!')
					menu.close()
				else
					ESX.ShowNotification('You cannot afford the taxi license.')
				end
			end)
		end
