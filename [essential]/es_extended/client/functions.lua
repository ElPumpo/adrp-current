...
ESX.Game.Utils.ProgressBar = function(completion, xoffset, yoffset, r, g, b, text)
	if xoffset == nil then
		xoffset = 0
	end
			...
			elseif data1.current.action == 'give_ammo' then
				...
				if IsPedOnFoot(closestPed) then
					if closestPlayer ~= -1 and closestDistance < 3.0 then
						if pedAmmo > 0 then
							ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_count_give', {
								title = _U('amountammo')
							}, function(data2, menu2)
								local quantity = tonumber(data2.value)
