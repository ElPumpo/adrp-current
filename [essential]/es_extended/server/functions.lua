...
ESX.SavePlayer = function(xPlayer, cb)
	...
	-- Job, loadout, items and position
	table.insert(asyncTasks, function(cb)
		MySQL.Async.execute('UPDATE users SET job = @job, job_grade = @job_grade, gang = @gang, gang_grade = @gang_grade, inventory = @inventory, loadout = @loadout, position = @position WHERE identifier = @identifier', {
			['@job']        = xPlayer.job.name,
			['@job_grade']  = xPlayer.job.grade,
			['@gang']       = xPlayer.gang.name,
			['@gang_grade'] = xPlayer.gang.grade,
	...
	Async.parallel(asyncTasks, function(results)
		print(('[es_extended] [^2INFO^7] player %s has been saved'):format(xPlayer.identifier))
...
ESX.SavePlayers = function(cb)
	...
	Async.parallelLimit(asyncTasks, 5, function(results)
		print('[es_extended] [^2INFO^7] all players have been saved')
