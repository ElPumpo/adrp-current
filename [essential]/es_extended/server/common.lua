...
TriggerEvent("es:addGroup", "pr", "user", function(group) end)
...
MySQL.ready(function()
	...
	-- GANGS
	--
	local result = MySQL.Sync.fetchAll('SELECT * FROM gang', {})

	for i=1, #result do
		ESX.Gangs[result[i].name] = result[i]
		ESX.Gangs[result[i].name].grades = {}
	end

	local result2 = MySQL.Sync.fetchAll('SELECT * FROM gang_grades', {})

	for i=1, #result2 do
		if ESX.Gangs[result2[i].gang_name] then
			ESX.Gangs[result2[i].gang_name].grades[tostring(result2[i].grade)] = result2[i]
		else
			print(('es_extended: invalid gang "%s" from table gang_grades ignored!'):format(result2[i].job_name))
		end
	end
