...
local timer = 0
local currentPosition, previousPosition, currentHeading, previousHeading
			...
			if currentPosition == previousPosition and currentHeading == previousHeading then
				if timer > 0 then
					if timer == math.ceil(afkTimeout / 4) then
						TriggerEvent('chat:addMessage', { args = { '^1AFK WARNING', ('You will be kicked in %s seconds for being inactive!'):format(timer) } })
					end
