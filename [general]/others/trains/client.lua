...
function checkKillmetro()
	...
	if not dontdelete then
		...
		ESX.Game.DeleteVehicle(trailer)
		ESX.Game.DeleteVehicle(MetroTrain)
		stopsrunning=false
		imhost=false
	end
end
...
AddEventHandler("AskForTrainConfirmed", function()
	...
	local countryDist = GetDistanceBetweenCoords(closestCountry[1],closestCountry[2],closestCountry[3], GetEntityCoords(PlayerPedId()) )
	local metroDist = GetDistanceBetweenCoords(closestMetro[1],closestMetro[2],closestMetro[3], GetEntityCoords(PlayerPedId()) )

	if metroDist < countryDist then
		if metroDist < 25.0 then
			StartMetro(closestMetro)
			TriggerEvent('customNotificaiton', ' Choo Choo..')
		else
			TriggerEvent('customNotificaiton',' You are not near a train station..')
		end
	else
...
Citizen.CreateThread(function()
	for i = 1, #trainstations do
		Citizen.Wait(100)
		local blip = AddBlipForCoord(trainstations[i][1],trainstations[i][2],trainstations[i][3])
		...
		AddTextComponentSubstringPlayerName("Metro Train Station")
		EndTextCommandSetBlipName(blip)
	end

	for i = 1, #trainstations2 do
		local blip = AddBlipForCoord(trainstations2[i][1],trainstations2[i][2],trainstations2[i][3])
		...
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentSubstringPlayerName("Country Train Station")
		EndTextCommandSetBlipName(blip)
	end
end)
		...
		if(IsControlJustReleased(0, 38))then
			...
			if IsThisModelATrain( GetEntityModel(targetVehicle) ) and not intrain then
				...
				if IsPedInAnyTrain(playerped) then
					intrain = true
					TriggerEvent("customNotification","You boarded the train!")
				else
					TriggerEvent("customNotification","Sorry, this carriage is full!")
				end
				Citizen.Wait(1500)
				justborded = false
			else
...
function StartMetro(coords)
	...
	TriggerEvent('customNotification', 'Train will depart in 30 seconds.')
