...
local mh65c_hash = GetHashKey("mh65c")
local as350_hash = GetHashKey("as350")
local jayhawk_hash = GetHashKey("jayhawk")
local rcmavic_hash = GetHashKey("rcmavic")
...
Citizen.CreateThread(function()
	while true do
		...
		local isInPolMav = IsPlayerInPolmav()

		if not isInPolMav then
			Citizen.Wait(500)
		end
		...
		if isInPolMav and target_vehicle and not helicam and vehicle_display ~=2 then
...
function IsPlayerInPolmav()
	...
	local isas350 = IsVehicleModel(vehicle, as350_hash)
	local isjayhawk = IsVehicleModel(vehicle, jayhawk_hash)
	local isrcmavic = IsVehicleModel(vehicle, rcmavic_hash)
	local isCorrect = false
	if(Ismh65c or isas350 or isjayhawk or isrcmavic) then
		isCorrect = true
	end
		return isCorrect
end
...
function RenderVehicleInfo(vehicle)
	if DoesEntityExist(vehicle) then
		...
		local licenseplate = ESX.Math.Trim(GetVehicleNumberPlateText(vehicle))
